...
public class UIParticleSystem : MonoBehaviour {
    ...
    /// <summary>
    /// Max Particle Emission Speed
    /// </summary>
    public float MaxSpeed = 5f;

    /// <summary>
    /// Min Particle Emission Speed
    /// </summary>
    ...
    private IEnumerator CoParticleFly(Image particle)
    {
        ...
        GradientColorKey[] keys = RandomColor.colorKeys;
        Color c = keys[Random.Range(0, keys.Length)].color;
        float s = Random.Range(MinSpeed, MaxSpeed);
        float r = Random.Range(0, Rotation);
        ...
        while (particleLifetime < Lifetime)
        {
            ...
            //set position
            particle.transform.position += (emissonAngle * s + gravityForce) * SpeedOverLifetime.Evaluate(particleLifetime / Lifetime);

            //set scale
            particle.transform.localScale = Vector3.one * SizeOverLifetime.Evaluate(particleLifetime / Lifetime) * Size;

            //set rortaion
            particle.transform.localRotation = Quaternion.AngleAxis(r * particleLifetime, Vector3.forward);
